...
def main():
    ...
    USERNAME = str(getenv("USERNAME_BO"))
    PASSWORD = str(getenv("PASSWORD_BO"))
    ...
    def save_excel_response(resp):
        ...
        filename = match.group(1) if match else "error.html"
        try:
            create = open(filename,"x")
            create.close()
            save_excel_response(resp)
        except FileExistsError:
            with open(filename,"wb") as f:
                f.write(resp.content)
            print(f"Reporte exitoso en: {filename}")
